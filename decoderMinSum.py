...
def belief_propagation_decode_min_sum(llr, H, max_iter=50, alpha=0.75):
    """
    Dekodowanie LDPC metodą Min-Sum z opcjonalną korekcją skalującą (alpha)
    Stosuję się go ponieważ jest mniejsz "kosztowny" przy czym szybszy, jest to jedank kosztem dokładności
    ...
    Parametry:
      ...
      alpha    – współczynnik korekcji skalującej (np. 0.75–0.95) średnio jest z przedziału 0.6 - 0.9 ale głównie 0.75-0.8, kompensuje przybliżenie.
    ...
    for iteration in range(max_iter):
        # Krok 1: aktualizacja wiadomości od check nodes do zmiennych – wersja Min-Sum
        for i in range(m):
            for j in check_nodes[i]:
                msgs = [q[i, j2] for j2 in check_nodes[i] if j2 != j]
                if not msgs:
                    continue
                signs = np.sign(msgs)
                min_abs = np.min(np.abs(msgs))
                sign_product = np.prod(signs)
                r[i, j] = alpha * min_abs * sign_product
        ...
        for j in range(n):
            for i in var_nodes[j]:
                ...
                sum_r = sum(r[i2, j] for i2 in var_nodes[j] if i2 != i)
                updated_llr[j] = llr[j] + sum_r
                q[i, j] = llr[j] + sum_r

        # Estymacja bitów: jeśli LLR < 0, to bit = 1, w przeciwnym razie = 0
...
if __name__ == '__main__':
    # Test dekodera Min-Sum z przykładowymi LLR
    from ldpc_matrices import get_ldpc_matrix
    H, _ = get_ldpc_matrix('648_1/2')
    llr = np.random.randn(12)
    decoded_bits = belief_propagation_decode_min_sum(llr, H, max_iter=50, alpha=0.75)
